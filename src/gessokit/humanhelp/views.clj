...
(defn user-menu
  ...
  [:details {:class "relative"}
   ...
   [:div {:class "absolute right-0 z-50 mt-2 min-w-48 radius-md border-theme pad-panel shadow-lg"
          :style {:border-style "solid"
                  :border-color "var(--border)"
    ...
    [:div {:class "content-stack-theme"}
     [:div {:class "font-body text-xs-theme leading-body"
            :style {:color "var(--muted-foreground)"}}
      "Signed in as"]
     [:div {:class "font-body text-sm-theme leading-body weight-medium-theme"}
      (user-email user)]
...
(defn page
  ...
  (ui/page-shell
   ...
   [:div {:class "relative isolate min-h-screen"
    ...
    (g/bars
     {:brand (brand)
      :center (ui/theme-dialog ctx {:trigger-label? false})
      :end (user-menu user)}
     [:main {:class "mx-auto w-full max-w-2xl px-4 py-8 sm:px-6 lg:px-8"}
      [:div {:class "content-stack-theme gap-section"}
       ...
       (create-request-dialog ctx {:user user})]])]))
...
(defn request-toolbar-fragment
  ...
  [:div {:id request-toolbar-dom-id
   ...
   [:div {:class "cluster-theme items-center justify-between"}
    [:div {:class "cluster-theme items-baseline"}
     [:h2 {:class "font-heading leading-heading tracking-heading text-2xl-theme weight-semibold-theme"}
      "Requests"]
     [:span {:class "font-body text-sm-theme leading-body"
             :style {:color (if stale?
                              "var(--primary)"
                              "var(--muted-foreground)")}}
      "(" open-count " Open"
      (when (pos? (or pending-open-count 0))
        (str ", +" pending-open-count " new"))
      ")"]]
    ...
    [:div {:class "cluster-theme items-center"}
     [:form {:method "post"
      ...
      [:button {:type "submit"
                ...
                :style {:border-style "solid"
                        :border-color (if stale? "var(--primary)" "var(--border)")
                        :background (if stale? "var(--primary)" "var(--card)")
                        :color (if stale? "var(--primary-foreground)" "var(--card-foreground)")}}
       (g/icon "refresh-cw" {:size :sm})
       [:span "Refresh"]]]

     [:button {:type "button"
               :class "inline-flex items-center justify-center control-theme radius-md border-theme"
               :style {:border-style "solid"
                       :border-color "var(--border)"
   ...
   (when stale?
     [:div {:class "radius-md pad-row"
            :style {:background "color-mix(in srgb, var(--primary) 14%, transparent)"
                    :color "var(--foreground)"}}
      [:div {:class "cluster-theme items-center"}
       (g/icon "sparkles" {:size :sm})
       [:span {:class "font-body text-sm-theme leading-body"}
        "New request data is available. Refresh when you are ready."]]])])
...
(defn search-control
  ...
  [:form {:id board-state-form-id
          ...
          :class "content-stack-theme"}
   (hidden-input routes/visible-revision-param (:visible-revision view-state))
   (hidden-input routes/selected-param (:selected-request-id view-state))
...
(defn request-card-style
  [selected?]
  {:border-style "solid"
   :border-color (if selected? "var(--primary)" "var(--border)")
   :background "var(--background)"
   :color "var(--foreground)"
...
(defn request-meta
  ...
  [:div {:class "cluster-theme items-center"}
   ...
   [:span {:class "font-body text-xs-theme"
           :style {:color "var(--muted-foreground)"}}
    "waiting " (domain/waiting-label request)]])
...
(defn request-card-actions
  ...
  (let [actions (domain/available-actions request user)]
    (when (seq actions)
      [:div {:class "cluster-theme items-center"}
       (for [action actions]
         (action-button ctx request user action view-state))])))
...
(defn request-card
  ...
  (let [selected? (card-selected? request view-state)]
    [:article {:id (str "humanhelp-request-" (:request/id request))
     ...
     [:a {:href (routes/select-request-url (:request/id request) view-state)
          :hx-get (routes/select-request-url (:request/id request) view-state)
          :hx-target (str "#" request-list-dom-id)
          :hx-swap "outerHTML"
...
(defn field-error
  ...
  (when-let [error (get errors k)]
    [:p {:class "font-body text-xs-theme leading-body"
...
(defn request-action-error
  [_ctx {:keys [result]}]
  (let [message (or (get-in result [:error :message])
