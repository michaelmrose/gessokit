(ns gessokit.app
  "HTTP boundary for the Human Help analogue app.
...
(defn param
  "Read a Ring/Biff request param by keyword or string key.
   ...
   This stays in app.clj because it is an HTTP boundary concern."
  [ctx k]
...
(defn create-request-dialog-fragment
  [ctx]
  (html
   (views/create-request-dialog-content
    ctx
    {:user (current-user ctx)
     :values {}
     :errors {}})))
...
(defn create-request-success-response
  ...
  (let [view-state'  (assoc view-state :visible-revision revision)
        toolbar      (render-toolbar-node ctx view-state')
        request-list (render-list-node ctx view-state')]
    (html
     (views/oob-response
      (g/oob-outer-html views/request-toolbar-dom-id toolbar)
      (g/oob-outer-html views/request-list-dom-id request-list)
      [:div {:id views/create-request-dialog-body-id}
       [:script
...
(defn create-request!
  "Create a new request from the modal dialog.
   ...
   - receive a toast through client plumbing
  ...
  (let [user       (current-user ctx)
    ...
    (if (seq errors)
      ...
      (let [{:keys [request revision]}
        ...
        (client-plumbing/send-humanhelp-new-request-toast!
...
(defn reset-demo!
  ...
  (let [user       (current-user ctx)
    ...
    (client-plumbing/send-toast-to-scope!
     client-plumbing/humanhelp-store-scope
     {:variant :info
      :title "Demo reset"
