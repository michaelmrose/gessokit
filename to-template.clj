...
(defn copy-template-file!
  ...
  (let [rel         (rel-str source-root p)
    ...
    (cond
      ;; Binary / raw file with unchanged path:
      ;; Keep it out of root so deps-new does not perform text replacement.
      ...
      ;; Binary / raw file whose path depends on source-primary-ns:
      ;; Use :raw plus an explicit rename map.
      (and binary? rename?)
      (do
      ...
      ;; Text file whose path depends on source-primary-ns:
      ;; Stage it in rename/ and generate a :transform map.
      rename?
      ...
      ;; Normal text file:
      ;; Put it under root/ and let deps-new copy/substitute it normally.
      :else
      (if-let [txt (read-text-safe p)]
        (do
          (write-text-file!
...
(defn template-edn [rename-map raw? raw-rename-map]
  (let [transforms (cond-> []
                     ...
                     raw?
                     (conj (transform-entry "raw" nil [:raw]))

                     (seq raw-rename-map)
                     (conj (transform-entry "raw-rename" raw-rename-map [:only :raw])))]
...
(defn write-readme! [template-dir template-name template-resource-dir]
  (write-text-file!
   ...
   (str "# Generated deps-new Template\n\n"
        ...
        "Template EDN:\n\n"
        "```text\n"
        (path-str (fs/relativize template-dir
                                  (fs/path template-resource-dir "template.edn")))
        "\n```\n")))
         ...
         (keep
          (fn [p]
            (when-let [txt (read-text-safe p)]
              (let [hits (for [[idx line] (map-indexed vector (str/split-lines txt))
...
(defn warn-leftovers! [dirs source-primary-ns]
  (let [leftovers (remaining-content-matches dirs source-primary-ns)]
    (when (seq leftovers)
      (binding [*out* *err*]
        (println)
        (println "WARNING: old primary namespace text remains in template file content:")
...
(defn main [& args]
  (let [[template-dir-name source-primary-ns template-name & more] args]
    ...
    (let [source-root           (fs/cwd)
          template-name         (or template-name
                                    (default-template-name template-dir-name))
          template-dir          (sibling-path source-root template-dir-name)
          template-resource-dir (apply fs/path
                                       template-dir
                                       "resources"
                                       (template-path-parts template-name))
          root-dir              (fs/path template-resource-dir "root")
          rename-dir            (fs/path template-resource-dir "rename")
          raw-dir               (fs/path template-resource-dir "raw")
      ...
      (let [ctx {:source-root       source-root
            ...
            rename-map (into (sorted-map)
                             (for [{:keys [kind from to]} results
                                   :when (= :rename kind)]
                               [from to]))

            raw? (boolean
                  (some #(= :raw (:kind %)) results))

            raw-rename-map (into (sorted-map)
                                 (for [{:keys [kind from to]} results
                                       :when (= :raw-rename kind)]
                                   [from to]))]

        (write-template-edn! template-resource-dir rename-map raw? raw-rename-map)
        (write-deps-edn! template-dir)
        (write-readme! template-dir template-name template-resource-dir)

        ;; Do not scan template.edn here: its rename-map keys intentionally
        ;; contain original source paths.
        (warn-leftovers!
         [root-dir rename-dir raw-dir raw-rename-dir]
         source-primary-ns)
        ...
        (println " " (path-str (fs/path template-resource-dir "template.edn")))
        (println)
        (println "Use locally with:")
        (println
         (str "clojure -Sdeps '{:deps {local/template {:local/root \""
              (path-str template-dir)
              "\"}}}' "
              "-Tnew create "
              ":template " template-name " "
